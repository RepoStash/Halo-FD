	...
	for(var/mob/living/player in GLOB.mobs_in_sectors[src])
		if(istype(player.loc,/turf)) //There's a number of situations where being inside something may be a problem, so let's handle them all here.
			if(crash_landing)
				...
				to_chat(player,"<font size=4><span class = 'userdanger'>You hang on for dear life as [src] de-orbits.</span></font>")
				player.Stun(6)
				player.flash_eyes()
			else
				player.dust()
...
/obj/effect/overmap/proc/pre_superstructure_failing()
	for(var/mob/player in GLOB.mobs_in_sectors[src])
		if(istype(src,/obj/effect/overmap/sector))
			to_chat(player,"<font size=4><span class = 'danger'>Core AO Planetary Structure Failing. ETA: [SUPERSTRUCTURE_FAIL_TIME/600] minutes.</span></font>")
		else
			to_chat(player,"<font size=4><span class = 'danger'>SHIP SUPERSTRUCTURE FAILING. ETA: [SUPERSTRUCTURE_FAIL_TIME/600] minutes.</span></font>")
